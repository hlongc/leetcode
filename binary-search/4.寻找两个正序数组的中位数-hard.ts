...
function findMedianSortedArrays(nums1: number[], nums2: number[]): number {
  ...
  /**
     在这个算法中，partLen1 表示的是分割位置，而不是数组索引。具体来说：
    分割位置的含义
    partLen1 = 0：nums1 不贡献任何元素到左半部分
    partLen1 = 1：nums1 贡献 1 个元素（索引 0）到左半部分
    partLen1 = 2：nums1 贡献 2 个元素（索引 0,1）到左半部分
    partLen1 = len1：nums1 贡献所有元素到左半部分
    为什么 end = len1

    // 如果 nums1 = [1, 2, 3, 4]，len1 = 4
// 那么 partLen1 的可能取值是：0, 1, 2, 3, 4

partLen1 = 0: 左半部分 = [], 右半部分 = [1, 2, 3, 4]
partLen1 = 1: 左半部分 = [1], 右半部分 = [2, 3, 4]  
partLen1 = 2: 左半部分 = [1, 2], 右半部分 = [3, 4]
partLen1 = 3: 左半部分 = [1, 2, 3], 右半部分 = [4]
partLen1 = 4: 左半部分 = [1, 2, 3, 4], 右半部分 = []
     */
